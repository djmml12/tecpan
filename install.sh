...
MOBILE_SERVICE_NAME="tecpan-mobile"
MOBILE_SERVICE_SRC="$REPO_DIR/scripts/rpi/tecpan-mobile.service"
MOBILE_SERVICE_DST="/etc/systemd/system/$MOBILE_SERVICE_NAME.service"
...
info "Compilando frontend tablet..."
npm run build --workspace=apps/pos-tablet
ok "Frontend tablet compilado en apps/pos-tablet/dist"

info "Compilando frontend mobile..."
npm run build --workspace=apps/pos-mobile
ok "Frontend mobile compilado en apps/pos-mobile/dist"
...
info "Instalando servicio systemd '$SERVICE_NAME' (backend + tablet)..."
sed \
...
info "Instalando servicio systemd '$MOBILE_SERVICE_NAME' (mobile)..."
sed \
  -e "s|__REPO_DIR__|$REPO_DIR|g" \
  -e "s|__USER__|$USER|g" \
  "$MOBILE_SERVICE_SRC" | sudo tee "$MOBILE_SERVICE_DST" >/dev/null
...
sudo systemctl enable "$MOBILE_SERVICE_NAME"
sudo systemctl restart "$MOBILE_SERVICE_NAME"
...
if systemctl is-active --quiet "$MOBILE_SERVICE_NAME"; then
  ok "Servicio '$MOBILE_SERVICE_NAME' activo y habilitado en boot"
else
  warn "El servicio mobile no arrancó. Revisa: sudo journalctl -u $MOBILE_SERVICE_NAME -n 30"
fi
...
echo -e "${BOLD}══════════════════════════════════════════${RESET}"
echo ""
echo -e "  POS Tablet (caja):"
echo -e "  ${BOLD}${CYAN}http://$IP:3000${RESET}"
echo -e "  ${BOLD}${CYAN}http://$(hostname).local:3000${RESET}"
echo ""
echo -e "  POS Mobile (meseros / clientes):"
echo -e "  ${BOLD}${CYAN}http://$IP:5174${RESET}"
echo -e "  ${BOLD}${CYAN}http://$(hostname).local:5174${RESET}"
echo ""
